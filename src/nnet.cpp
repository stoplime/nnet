...
void back_propagation()
{
	...
	for (int i = 0; i < layer_nodes; i++)
	{
		for (int j = 0; j < layer_nodes; j++)
		{
			weight0[i][j] -= weight0[i][j]*error_respect_weight0[i][j]*learning_rate;
			weight1[i][j] -= weight1[i][j]*error_respect_weight1[i][j]*learning_rate;
			weight2[i][j] -= weight2[i][j]*error_respect_weight2[i][j]*learning_rate;
		}
		bias_weight[0][i] -= bias_weight[0][i]*error_respect_bias_weight0[i]*learning_rate;
		bias_weight[1][i] -= bias_weight[1][i]*error_respect_bias_weight1[i]*learning_rate;
		bias_weight[2][i] -= bias_weight[2][i]*error_respect_bias_weight2[i]*learning_rate;
	}
...
void test_result (float percentage)
{
	...
	for (int i = 0; i < layer_nodes; i++)
	{
		cout << pow(abs(output_layer[1][i] - target_ans_binary[i]), 2) << " ";
	}
	cout << endl;
}
